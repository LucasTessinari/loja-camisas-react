...
const ProductCard = ({ product }) => {
  ...
  const [loadedImages, setLoadedImages] = useState({}); // 👈 NOVO: controla quais imagens já carregaram
  ...
  // 👇 NOVO: marca a imagem como carregada quando o onLoad dispara
      ...
      <div className="relative h-64 bg-gray-50 overflow-hidden shrink-0">
        ...
        {images.map((img, index) => (
          <div key={index} className="absolute inset-0">
            ...
            {/* 👇 SKELETON: aparece enquanto a imagem não carregou */}
            {!loadedImages[index] && (
              <div className="w-full h-full bg-gray-200 animate-pulse" />
            )}

            {/* 👇 LAZY LOADING: loading="lazy" + só fica visível após carregar */}
            <img
              src={img}
              alt={product.name}
              loading="lazy"
              ...
              className={`absolute inset-0 w-full h-full object-cover mix-blend-multiply transition-opacity duration-500 ${
                index === currentImage && loadedImages[index]
                  ? 'opacity-100'
                  : 'opacity-0'
              }`}
            />
