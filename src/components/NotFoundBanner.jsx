...
const NotFoundBanner = () => {
  // Ajuste o número se precisar
  const WHATSAPP_NUMBER = "5511999999999";

  const handleWhatsappSubmit = () => {
    const text = document.getElementById("pedido-input-banner")?.value;
    if (text) {
      window.open(
        `https://wa.me/${WHATSAPP_NUMBER}?text=Olá! Estou procurando essa camisa: ${encodeURIComponent(text)}`,
        "_blank"
      );
    } else {
  ...
  return (
     <section className="bg-brand-primary py-16 text-white mt-32 w-full">
